...
function generateAppCache(resources, app) {
  ...
  resources.forEach(function(instance) {
    ...
    filelist(absolutePath).forEach(function(file) {
      ...
      if (path === 'module-map.json' || path === 'circus.json') {
        return;
      }
    ...
    var map = loadJSON(absolutePath + '/module-map.json')
          || loadJSON(absolutePath + '/circus.json');
    if (map && map.hulahoop) {
      _.each(map.hulahoop.modules, function(module) {
        function remap(entry) {
          entry.href = map.published[entry.href] || entry.href;
        }

        _.each(module.js, remap);
        _.each(module.css, remap);
      });
