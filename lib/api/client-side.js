...
exports = module.exports = {
  ...
  embeds: function(configVar, config, routeInfo, options) {
    routeInfo = routeInfo || {};
    config = config || '';
    ...
    function resolveHref(href) {
      ...
      if (options.resourceRoot && !/\/\//.test(href)) {
        return path.join(options.resourceRoot, href);
      } else {
        return href;
      }
    }
    ...
    var ret = {
      script: (routeInfo.js || []).map(function(script) {
        return '<script type="text/javascript" src="' + resolveHref(script.href) + '"'
            + (script.attr ? ' ' + script.attr : '') + '></script>\n';
      }).join(''),
      link: (routeInfo.css || []).map(function(link) {
        return '<link rel="stylesheet" href="' + resolveHref(link.href) + '"'
            + (link.attr ? ' ' + link.attr : '') + '>\n';
      }).join('')
    };
  ...
  inline: function(app, configVar, config, routeInfo, options, callback) {
    var embeds = exports.embeds(configVar, config, routeInfo, options);
